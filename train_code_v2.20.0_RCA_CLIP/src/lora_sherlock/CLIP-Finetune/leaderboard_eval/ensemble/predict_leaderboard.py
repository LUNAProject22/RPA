...
def parse_args():
	...
	parser.add_argument('--region_prompt', type=int, default=0, 
                        help='1001=Mixed Mode: R-CTX, R-CPT, R-CIR,R-Box'
                        '101=R-CTX'
                        '102=R-CPT'
                        '103=R-CIR'
                        '104=R-Box'
	...
	parser.add_argument('--load_model_from_2')
	...
	parser.add_argument('--clip_model_2',
			default='ViT-B-16',
			choices=['ViT-B-16-672x336', 'ViT-B-16-224x112', 'ViT-B-16-224x448', 'ViT-B-16-448x224', 'ViT-H-14-448x224', 'ViT-H-14-672x336','ViT-L-14-336x672','ViT-L-14-672x336', 'ViT-L-14-336x504', 'ViT-L-14-336', 'ViT-B-32', 'ViT-B-16', 'ViT-B-16-cust-384', 'RN50x16', 'RN50x64', 'xlm-roberta-large-ViT-H-14'])
	parser.add_argument(
			 "--pretrained",
...
def comparison_main(model, model_2, args, tokenizer, tokenizer_2):
	...
	dset = torch.utils.data.DataLoader(
		clip_inference_iterator.CLIPDatasetLeaderboard(instances, args, tokenizer, tokenizer_2),
	...
	with torch.no_grad():
		...
		for batch in bar:
			...
			# Model -2
			images_M2, captions_M2 = batch['image_M2'].to(args.device), batch['caption_M2'].to(args.device)
			box_images_M2 = batch['box_image_M2'].to(args.device)
			image_features_M2, text_features_M2 = model_2(box_images_M2, images_M2, captions_M2)
			if args.region_prompt == 1001:
				images2_M2= batch['image2_M2'].to(args.device)
				box2_images_M2 = batch['box2_image_M2'].to(args.device)
				image2_features_M2, text2_features_M2 = model_2(box2_images_M2, images2_M2, captions_M2)
				images3_M2= batch['image3_M2'].to(args.device)
				box3_images_M2 = batch['box3_image_M2'].to(args.device)
				image3_features_M2, text3_features_M2 = model_2(box3_images_M2, images3_M2, captions_M2)
				#images4= batch['image4'].to(args.device)
				#box4_images = batch['box4_image'].to(args.device)
				#image4_features, text4_features = model(box4_images, images4, captions)
				image_features_M2 = (image_features_M2 + image2_features_M2 + image3_features_M2) / 3.0
			image_features = torch.cat((image_features, image_features_M2),dim=-1)
			text_features = torch.cat((text_features, text_features_M2),dim=-1)
...
def main():
	...
	model_2, _, _ = open_clip.create_model_and_transforms(args.clip_model_2, 
                jit=False, pretrained=args.pretrained,
                Adapt_TxtEncoder=args.Adapt_TxtEncoder,
                Adapt_VisEncoder=args.Adapt_VisEncoder,
                adapter_rate=args.adapter_rate,
                ada_type=args.AdaType)
	tokenizer_2   = open_clip.get_tokenizer(args.clip_model_2)
	if args.load_model_from_2 != 'None':
		print('Getting model weights from {}'.format(args.load_model_from_2))
		state = torch.load(args.load_model_from_2, map_location=args.device)
		state_dict = state['state_dict']
	...
	try:
		args.input_resolution_2 = model_2.visual.image_size
	except:
		args.input_resolution_2 = model_2.input_resolution

	model_2 = clip_inference_iterator.CLIPExtractor(model_2, args)
	...
	if args.task == 'retrieval':
		...
		comparison_main(model, model_2, args, tokenizer, tokenizer_2)
