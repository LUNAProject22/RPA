...
class CLIPDatasetLeaderboard(torch.utils.data.Dataset):
	def __init__(self, data, args, tokenizer, tokenizer_2):
		self.args = args
		self.data = data
		...
		n_px_2 = min(args.input_resolution_2)
		self.preprocess_2 = self._transform_test((n_px_2, n_px_2))
	...
	def image_to_torch_tensor_2(self, image, box_images):
		trans_images = self.preprocess_2(image)

		rand_index = random.randrange(len(box_images))
		sig_box_image = box_images[rand_index]
		trans_box_images =  self.preprocess_2(sig_box_image)
		return trans_images, trans_box_images
	...
	def __getitem__(self, idx):
		...
		if self.args.region_prompt == 1001: # Mixed Mode
			...
			#image4, box_image4   = get_region_context_combo(image, reg)
		...
		caption_M2 = self.tokenizer_2('inference {}'.format(inference_raw)).squeeze()
		cid = c_data['instance_id']

		if self.args.region_prompt == 1001:
			image1_M1, box_image1_M1 = self.image_to_torch_tensor(image1, box_image1)
			image2_M1, box_image2_M1 = self.image_to_torch_tensor(image2, box_image2)
			image3_M1, box_image3_M1 = self.image_to_torch_tensor(image3, box_image3)

			image1_M2, box_image1_M2 = self.image_to_torch_tensor_2(image1, box_image1)
			image2_M2, box_image2_M2 = self.image_to_torch_tensor_2(image2, box_image2)
			image3_M2, box_image3_M2 = self.image_to_torch_tensor_2(image3, box_image3)
			#image4, box_image4 = self.image_to_torch_tensor(image4, box_image4)
			return {
                                'image':image1_M1, 'caption':caption, 'id': cid, 
                                'box_image': box_image1_M1, 'image2':image2_M1, 
                                'box2_image': box_image2_M1, 'image3':image3_M1, 
                                'box3_image': box_image3_M1,
                                'image_M2':image1_M2, 'caption_M2':caption_M2,
                                'box_image_M2': box_image1_M2, 'image2_M2':image2_M2, 
                                'box2_image_M2': box_image2_M2, 'image3_M2':image3_M2, 
                                'box3_image_M2': box_image3_M2,
                                }
								#'image4':image4,
...
class CLIPDatasetLocalizationLeaderboard(CLIPDatasetLeaderboard):
	...
	def __getitem__(self, idx):
		...
		if self.args.region_prompt == 1001:
			...
			return {'caption': caption, 'image': image1, 'box_image':box1_image,
                                'instance_ids': instance_ids, 'image_idxs': image_idxs, 
                                'cap_idxs': cap_idxs, 
	   			'image2': image2, 'box2_image':box2_image,
                                'image3': image3, 'box3_image':box3_image }
                                #'image4': image4, 'box4_image':box4_image,
                                #}
		else:
			image1	 = torch.stack(image1_list)
			box1_image = torch.stack(box1_image_list)
