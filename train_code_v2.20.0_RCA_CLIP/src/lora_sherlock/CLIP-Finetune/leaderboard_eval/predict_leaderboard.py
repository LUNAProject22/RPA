...
def comparison_main(model, args, tokenizer):
	...
	with torch.no_grad():
		...
		for batch in bar:
			...
			captions_input_ids = batch['caption_input_ids']
			#print("DEBIG {}".format(captions_input_ids))
			captions_input_ids = captions_input_ids.to(args.device)
			captions_attention_mask = batch['caption_attention_mask'].to(args.device)
			box_images = batch['box_image'].to(args.device)
			#image_features = model.image_forward(images, box_images)
			image_features = model.image_forward( box_images, images)
			#print("DEBUG feat {}, img {}".format(image_features.shape, images.shape))
			text_features = model.text_forward(captions_input_ids, captions_attention_mask)
			if args.region_prompt == 1001:
				images2= batch['image2'].to(args.device)
				box2_images = batch['box2_image'].to(args.device)
				image2_features = model.image_forward(images2, box2_images)
				images3= batch['image3'].to(args.device)
				box3_images = batch['box3_image'].to(args.device)
				image3_features = model.image_forward(images3, box3_images)
...
def localization_main(model, args, tokenizer):
	...
	with torch.no_grad():
		...
		for batch in bar:
			...
			captions_input_ids = batch['caption_input_ids']
			captions_attention_mask = batch['caption_attention_mask']
			#print("DEBIG {}".format(captions_input_ids))
			txt_embs = []
			for i in range(len(captions_input_ids)):
			    input_ids = captions_input_ids[i].to(args.device)
			    attention_mask = captions_attention_mask[i].to(args.device)
			    txt_embs.append(model.text_forward(input_ids, attention_mask).squeeze(0))
			txt_embs = torch.stack(txt_embs)
			#print("TXT_EMBS {}".format(txt_embs.shape))
...
from load_lora import load_lora_model
def main():
	args = parse_args()
	np.random.seed(1)
	args.device = 'cuda' if torch.cuda.is_available() else 'cpu'
	#model, preprocess = clip.load(args.clip_model, jit=False)
	pretrained_lora_path = "./output/clip_ft_lora_all_e2/pretrained"
	model, processor, tokenizer, image_processor = load_lora_model(pretrained_lora_path)
#	model_bk, _, _ = open_clip.create_model_and_transforms(args.clip_model, 
...
#		ada_type=args.AdaType)
	#tokenizer_bk   = open_clip.get_tokenizer(args.clip_model)
