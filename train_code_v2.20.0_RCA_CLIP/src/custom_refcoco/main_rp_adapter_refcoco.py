...
from refcoco_dataloader import CLIPDataset

from refcoco_params import parse_args
from train_rp_adapter_refcoco import train_one_epoch, evaluate_refcoco
...
def main(args):
    ...
    with open(args.val_data) as f:
        val_f = json.load(f)
    with open(args.val_data_2) as f:
        val_f_2 = json.load(f)
    with open(args.val_data_3) as f:
        val_f_3 = json.load(f)
    ...
    val_data = CLIPDataset(val_f, args,training=False, 
        tokenizer=tokenizer, data_mean=data_mean, data_std=data_std)
    val_data_2 = CLIPDataset(val_f_2, args,training=False, 
        tokenizer=tokenizer, data_mean=data_mean, data_std=data_std)
    val_data_3 = CLIPDataset(val_f_3, args,training=False, 
        tokenizer=tokenizer, data_mean=data_mean, data_std=data_std)
    ...
    val_loader_2 = torch.utils.data.DataLoader(val_data_2, 
        batch_size=args.val_batch_size,num_workers=args.workers,
        pin_memory=False, sampler=val_sampler, drop_last=False)
    val_loader_3 = torch.utils.data.DataLoader(val_data_3, 
        batch_size=args.val_batch_size,num_workers=args.workers,
        pin_memory=False, sampler=val_sampler, drop_last=False)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == args.epochs - 1 or epoch % args.val_frequency == 0:
            evaluate_refcoco(model, val_data, val_loader, val_sampler, completed_epoch, args, writer, name='testA')
            evaluate_refcoco(model, val_data_2, val_loader_2, val_sampler, completed_epoch, args, writer, name='testB')
            evaluate_refcoco(model, val_data_3, val_loader_3, val_sampler, completed_epoch, args, writer,name='val')
